...
public class CDisableChallengePacket {
    private final ResourceLocation challenge;

    public CDisableChallengePacket(PacketBuffer buffer) {
        this.challenge = buffer.readResourceLocation();
    }

    public CDisableChallengePacket(ResourceLocation challenge) {
    ...
    private <T> T handle0(Supplier<NetworkEvent.Context> context) {
        context.get().enqueueWork(() -> {
            if (context.get().getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
                ...
                Network.sendToAllClients(new SDisableChallengePacket(this.challenge));
            }
        });
